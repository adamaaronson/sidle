...
export default class MultiPlayer extends Player {
    ...
    override isTopTouching(blocks: Entity[]) {
        return this.subentities.some(entity => blocks.some(block => 
            entity.top === block.bottom && entity.right > block.left && entity.left < block.right
        ))
    }

    override isBottomTouching(blocks: Entity[]) {
        return this.subentities.some(entity => blocks.some(block => 
            entity.bottom === block.top && entity.right > block.left && entity.left < block.right
        ))
    }

    override isLeftTouching(blocks: Entity[]) {
        return this.subentities.some(entity => blocks.some(block => 
            entity.left === block.right && entity.bottom > block.top && entity.top < block.bottom
        ))
    }

    override isRightTouching(blocks: Entity[]) {
        return this.subentities.some(entity => blocks.some(block => 
            entity.right === block.left && entity.bottom > block.top && entity.top < block.bottom
        ))
    }
    ...
    override isTopLeftTouching(blocks: Entity[]) {
        return this.subentities.some(entity => blocks.some(block => 
            entity.top === block.bottom && entity.left === block.right
        ))
    }

    override isTopRightTouching(blocks: Entity[]) {
        return this.subentities.some(entity => blocks.some(block => 
            entity.top === block.bottom && entity.right === block.left
        ))
    }

    override isBottomLeftTouching(blocks: Entity[]) {
        return this.subentities.some(entity => blocks.some(block => 
            entity.bottom === block.top && entity.left === block.right
        ))
    }

    override isBottomRightTouching(blocks: Entity[]) {
        return this.subentities.some(entity => blocks.some(block => 
            entity.bottom === block.top && entity.right === block.left
        ))
    }
}